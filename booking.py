...
from keyboards.keyboards import (
    calendar_kb, slots_kb, confirm_kb, cancel_confirm_kb,
...
async def cb_book_start(call: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if not subscribed:
        ...
        await call.answer()
        return
    await _show_calendar(call, state)
...
async def _show_calendar(call: CallbackQuery, state: FSMContext):
    ...
    if existing:
        await call.message.edit_text(
            ...
            parse_mode="HTML",
            reply_markup=main_menu_kb(is_admin=call.from_user.id == config.ADMIN_ID),
        )
        await call.answer()
        return
...
# Навигация по календарю тоже работает в состоянии choosing_date
@router.callback_query(BookingStates.choosing_date, F.data.startswith("cal:"))
async def cb_calendar_nav_booking(call: CallbackQuery):
...
async def cb_confirm(call: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    slot = get_slot(data["slot_id"])
    if not slot or slot["is_booked"]:
        await call.message.edit_text("⚠️ Слот уже занят. Выберите другое время.",
            reply_markup=main_menu_kb(is_admin=call.from_user.id == config.ADMIN_ID))
        await state.clear()
        await call.answer()
        return
    ...
    if not success:
        await call.message.edit_text("⚠️ Не удалось создать запись. Попробуйте позже.",
            reply_markup=main_menu_kb(is_admin=call.from_user.id == config.ADMIN_ID))
        await state.clear()
        await call.answer()
        return
    ...
    if booking:
        schedule_reminder(bot=bot, booking_id=booking["id"],
            user_id=call.from_user.id, date_str=slot["date"], time_str=slot["time"])
    ...
    try:
        await bot.send_message(config.ADMIN_ID,
            ...
            f"📅 Дата: <b>{slot['date']}</b>\n"
            f"🕐 Время: <b>{slot['time']}</b>", parse_mode="HTML")
    except Exception:
        pass
...
async def cb_cancel_my_booking(call: CallbackQuery):
    booking = get_user_booking(call.from_user.id)
    if not booking:
        await call.message.edit_text("ℹ️ У вас нет активных записей.", parse_mode="HTML",
            reply_markup=main_menu_kb(is_admin=call.from_user.id == config.ADMIN_ID))
        await call.answer()
        return
    await call.message.edit_text(
        ...
        parse_mode="HTML", reply_markup=cancel_confirm_kb(),
    )
    await call.answer()
...
async def cb_do_cancel(call: CallbackQuery, bot: Bot):
    booking = get_user_booking(call.from_user.id)
    if not booking:
        await call.message.edit_text("ℹ️ Запись не найдена.",
            reply_markup=main_menu_kb(is_admin=call.from_user.id == config.ADMIN_ID))
        await call.answer()
        return
    ...
    await call.message.edit_text("✅ <b>Ваша запись отменена.</b>", parse_mode="HTML",
        reply_markup=main_menu_kb(is_admin=call.from_user.id == config.ADMIN_ID))
    try:
        await bot.send_message(config.ADMIN_ID,
            f"❌ <b>Отмена записи клиентом</b>\n\n"
            f"🆔 TG ID: <code>{call.from_user.id}</code>\n"
            f"📅 Дата: <b>{date}</b>", parse_mode="HTML")
    except Exception:
        pass
