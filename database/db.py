...
def get_working_days() -> list:
    with get_connection() as conn:
        return conn.execute("SELECT * FROM working_days WHERE is_closed = 0 ORDER BY date").fetchall()
...
def get_day(date: str):
    with get_connection() as conn:
        return conn.execute("SELECT * FROM working_days WHERE date = ?", (date,)).fetchone()

def add_time_slot(date: str, time: str) -> bool:
    with get_connection() as conn:
        day = conn.execute("SELECT id FROM working_days WHERE date = ?", (date,)).fetchone()
        if not day:
            return False
        try:
            conn.execute("INSERT INTO time_slots (day_id, time) VALUES (?, ?)", (day["id"], time))
            return True
        except Exception:
            return False
...
def cancel_booking_by_user(user_id: int):
    with get_connection() as conn:
        booking = conn.execute("SELECT * FROM bookings WHERE user_id = ?", (user_id,)).fetchone()
        if booking:
            conn.execute("UPDATE time_slots SET is_booked = 0 WHERE id = ?", (booking["slot_id"],))
            conn.execute("DELETE FROM bookings WHERE id = ?", (booking["id"],))
        return booking

def cancel_booking_by_slot(slot_id: int):
    with get_connection() as conn:
        booking = conn.execute("SELECT * FROM bookings WHERE slot_id = ?", (slot_id,)).fetchone()
        if booking:
            conn.execute("UPDATE time_slots SET is_booked = 0 WHERE id = ?", (slot_id,))
            conn.execute("DELETE FROM bookings WHERE id = ?", (booking["id"],))
        return booking

def get_booking_for_slot(slot_id: int):
    with get_connection() as conn:
        return conn.execute("SELECT * FROM bookings WHERE slot_id = ?", (slot_id,)).fetchone()

def get_all_bookings_for_date(date: str) -> list:
    with get_connection() as conn:
        return conn.execute(
            """SELECT b.*, ts.time, wd.date FROM bookings b
            JOIN time_slots ts ON b.slot_id = ts.id
...
def get_all_future_bookings() -> list:
    with get_connection() as conn:
        return conn.execute(
            """SELECT b.*, ts.time, wd.date FROM bookings b
            JOIN time_slots ts ON b.slot_id = ts.id
